...
test_that("output_pb_gamlss_interactions", {
  model_e3_bw.DS <- ds.gamlss(formula = e3_bw ~ pb(e3_gac_None) + hs_correct_raven*hs_bmi_c_cat,
                              min.values = min(ds.test_env$local.values$e3_gac_None), 
                              max.values = max(ds.test_env$local.values$e3_gac_None), 
                              min.max.names = "e3_gac_None",
                              data = 'D', family = 'NO()')
  pb <- utils::getFromNamespace("pb", "gamlss")
  model_e3_bw <- gamlss::gamlss(formula = e3_bw ~ pb(e3_gac_None) + hs_correct_raven*hs_bmi_c_cat,
                                data = ds.test_env$local.values,
...
test_that("output_pb_gamlss_pbcontrol", {
  model_e3_bw.DS <- ds.gamlss(formula = e3_bw ~ pb(e3_gac_None, control=pb.control(inter=10)),
                              sigma.formula = ~ e3_gac_None,
                              min.values = min(ds.test_env$local.values$e3_gac_None), 
                              max.values = max(ds.test_env$local.values$e3_gac_None), 
                              min.max.names = "e3_gac_None",
  ...
  model_e3_bw <- gamlss::gamlss(formula = e3_bw ~ pb(e3_gac_None, control=pb.control(inter=10)),
                                sigma.formula = e3_bw ~ e3_gac_None,
                                data = ds.test_env$local.values,
                                family = gamlss.dist::NO())
  expect_length(model_e3_bw.DS, length(model_e3_bw)+1)
