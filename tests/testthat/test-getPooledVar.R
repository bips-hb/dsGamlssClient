...
test_that("output_getPooledVar_D_red$e3_bw", {
  ...
  weighted.var1 <- nrow(ds.test_env$gamlss_red)*var(ds.test_env$gamlss_red$e3_bw)
  pooled.var <- weighted.var1/nrow(ds.test_env$gamlss_red)
  expect_length(class(res), 1)
  expect_true(is.numeric(res))
  expect_equal(res, pooled.var, tolerance=1e-07)
})
...
test_that("output_getPooledVar_D_red$e3_gac_None", {
  ...
  weighted.var1 <- nrow(ds.test_env$gamlss_red)*var(ds.test_env$gamlss_red$e3_gac_None)
  pooled.var <- weighted.var1/nrow(ds.test_env$gamlss_red)
  expect_length(class(res), 1)
  expect_true(is.numeric(res))
  expect_equal(res, pooled.var, tolerance=1e-07)
