...
test_that("output_gamlssChecks_withprefix", {
  res <- gamlssChecks(formula=stats::as.formula(D$e3_bw ~ pb(D$e3_gac_None)), 
...
test_that("output_gamlssChecks_noprefix", {
  res <- gamlssChecks(formula=stats::as.formula(e3_bw ~ pb(e3_gac_None)), 
                      sigma.formula=stats::as.formula(~1), 
                      nu.formula=stats::as.formula(~1), 
                      tau.formula=stats::as.formula(~1), 
                      data='D', 
...
test_that("output_gamlssChecks_noprefix_dataNULL", {
  expect_error(gamlssChecks(formula=stats::as.formula(e3_bw ~ pb(e3_gac_None)), 
                            sigma.formula=stats::as.formula(~1), 
                            nu.formula=stats::as.formula(~1), 
                            tau.formula=stats::as.formula(~1), 
                            data=NULL,
                            datasources=ds.test_env$connections), 
               "No data.frame for the column e3_bw given. Specify it explicitly as dataname$e3_bw or provide a valid data argument.", fixed=TRUE)
})
