...
#Currently it is only possible to use opal and dslite for testing

source(testthat::test_path("connection_to_datasets", "init_local_settings.R"))
...
if (! is.null(getOption("default_driver"))) {
    ...
    # switch between "DSLiteDriver" and "OpalDriver" to test
...
if ((ds.test_env$driver == "DSLiteDriver") || (ds.test_env$driver == "OpalDriver")) {
    ds.test_env$ping_address <- paste("https://", ds.test_env$server_ip_address, ".de", sep="")
    ds.test_env$ping_config  <- config(timeout=5, ssl_verifyhost=0, ssl_verifypeer=0)

    ds.test_env$ip_address_1 <- paste("https://", ds.test_env$server_ip_address, ".de", sep="")
    ds.test_env$ip_address_2 <- paste("https://", ds.test_env$server_ip_address, ".de", sep="")
    ds.test_env$ip_address_3 <- paste("https://", ds.test_env$server_ip_address, ".de", sep="")

    ds.test_env$user_1 <- getOption("opal.user", "username")
    ds.test_env$user_2 <- getOption("opal.user", "username")
    ds.test_env$user_3 <- getOption("opal.user", "username")
